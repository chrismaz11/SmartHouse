...
class WiFiTriangulationApp {
    ...
    setButtonLoading(buttonId, isLoading) {
        ...
        if (isLoading) {
            ...
            // Use inline styles for spinner to ensure contrast on all button types
            btn.innerHTML = '<div class="spinner" style="border-color: rgba(255,255,255,0.3); border-top-color: white; margin: 0 auto;"></div>';
        } else {
            btn.disabled = false;
    ...
    async scanNetworks(fromUser = false) {
        ...
        try {
            this.networks = await window.electron.invoke('scan-wifi');
            this.accessPoints = await window.electron.invoke('get-access-points');
    ...
    async refreshDevices(fromUser = false) {
        ...
        try {
            this.devices = await window.electron.invoke('get-devices');
            const devicePositions = await window.electron.invoke('get-device-positions');
    ...
    updateDeviceTagging() {
        ...
        container.querySelectorAll('input').forEach(input => {
            input.addEventListener('blur', async () => {
                ...
                await window.electron.invoke('save-device-tags', tags);
            });
        });
    }
    ...
    async loadAutomations() {
        try {
            this.automations = await window.electron.invoke('get-automations');
    ...
    async createAutomation() {
        ...
        await window.electron.invoke('save-automation', automation);
        await this.loadAutomations();
        this.hideAutomationModal();
    }
    ...
    async deleteAutomation(id) {
        ...
        try {
            ...
            await window.electron.invoke('delete-automation', id);
            await this.loadAutomations();
        } catch (error) {
    ...
    async testSmartDevice(deviceType) {
        try {
            const result = await window.electron.invoke('test-smart-device', deviceType, 'test');
            if (result) {
    ...
    async loadSettings() {
        try {
            const settings = await window.electron.invoke('load-settings');
    ...
    startPeriodicUpdates() {
        ...
        window.electron.on('gemini-improvements', (improvements) => {
    ...
    async startIntelligentSetup() {
        ...
        try {
            const response = await window.electron.invoke('start-intelligent-setup');
            if (response) {
    ...
    async continueIntelligentSetup() {
        try {
            const response = await window.electron.invoke('process-user-action', 'continue', {
    ...
    async markCurrentLocation() {
        ...
        try {
            const response = await window.electron.invoke('walking-setup', position, deviceType, setupCodes);
    ...
    async updateSetupProgress() {
        try {
            const progress = await window.electron.invoke('get-setup-progress');
            if (progress) {
