...
## 2026-01-21 - [Efficient String Escaping in Render Loops]
**Learning:** Frequent DOM updates using `innerHTML` with inefficient string escaping (chained `.replace()`) can cause measurable CPU overhead in the renderer process.
**Action:** Use single-pass regex replacement with a lookup map for sanitization functions used in tight loops or list rendering.
