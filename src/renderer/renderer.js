...
class WiFiTriangulationApp {
    ...
    setButtonLoading(btn, isLoading) {
        ...
        if (isLoading) {
            ...
            // Ensure contrast for primary buttons (white spinner)
            const isPrimary = btn.classList.contains('btn-primary');
            const spinnerColor = isPrimary ? '#ffffff' : '#3b82f6';
            const spinnerBorder = isPrimary ? 'rgba(255,255,255,0.3)' : '#334155';

            btn.innerHTML = `
                <div class="spinner" style="border-color: ${spinnerBorder}; border-top-color: ${spinnerColor}; display: inline-block; vertical-align: middle; margin-right: 8px;"></div>
                ${btn.innerText}
            `;
        } else {
