...
class WiFiTriangulationApp {
    ...
    setButtonLoading(btn, isLoading) {
        ...
        if (isLoading) {
            ...
            // Adjust spinner colors for contrast
            const isPrimary = btn.classList.contains('btn-primary');
            const borderColor = isPrimary ? 'rgba(255,255,255,0.3)' : '#334155';
            const topColor = isPrimary ? '#ffffff' : '#3b82f6';

            btn.innerHTML = `<div class="spinner" style="border-color: ${borderColor}; border-top-color: ${topColor}; margin: 0 auto;"></div>`;
        } else {
